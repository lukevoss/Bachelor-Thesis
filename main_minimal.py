...
dirnum = 172 # attempt number, to avoid over writing data
...
train_loader = torch.utils.data.DataLoader(dd_train, shuffle = True, batch_size=batch_size, drop_last = True)
test_loader = torch.utils.data.DataLoader(dd_test, shuffle = True, batch_size=batch_size, drop_last = True)
...
try:
  ...
  for epoch in range(num_epochs):
    loss, img_t,dose_t, output_t= train()
    loss_train.append(loss)
    loss, img, dose, output = test()
